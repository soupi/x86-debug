...
tests =
  testGroup "Simple" $
    mconcat
      ...
      , zipWith (\n t -> testCase ("Simple " ++ show n) t) [1..] $ map apply simple
      , zipWith (\n t -> testCase ("Jumps  " ++ show n) t) [1..] $ map apply jumps
      , zipWith (\n t -> testCase ("Stack  " ++ show n) t) [1..] $ map apply stack
      , zipWith (\n t -> testCase ("Calls  " ++ show n) t) [1..] $ map apply calls
      ]
...
simple :: [([Instruction] -> Assertion, [Instruction])]
simple =
  [ ( testReg EAX 7
...
jumps :: [([Instruction] -> Assertion, [Instruction])]
jumps =
  [ ( testReg EAX 7
...
stack :: [([Instruction] -> Assertion, [Instruction])]
stack =
  [ ( testReg EAX 7
    , [ IMov  (AE $ Var EBX) (AE $ Lit 8)
      , IMov  (AE $ Var EAX) (AE $ Lit 1)
...
calls :: [([Instruction] -> Assertion, [Instruction])]
calls =
  [ ( testReg EAX 7
    , [ ICall (Var (AL "set"))
      , IJmp (Var (AL "end"))
...
testReg reg val code =
  assertEq'
    (getReg reg)
    (getMachine =<< mToE (interpret [initMachine $ toCode [] code]))
    (pure val)
